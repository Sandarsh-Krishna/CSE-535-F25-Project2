...
@Composable
fun NavRoot(nav: NavHostController, vm: GameViewModel, p2pVm: P2PGameViewModel) {
    NavHost(navController = nav, startDestination = AppRoute.MODE_SELECT.name) {
        ...
        composable(AppRoute.P2P.name) {
            P2PGameScreen(nav = nav, gameVm = vm, p2pVm = p2pVm)
        }
    }
}
